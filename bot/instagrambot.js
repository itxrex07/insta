...
export class InstagramBot {
  constructor() {
    ...
    this.mediaHandlers = [];
    this.sessionPath = './session/instagram_session.json';
  }
  ...
  async saveSession() {
    try {
      ...
      await fs.writeJson(this.sessionPath, serialized);
  ...
  startMessageListener() {
    // This is a simplified message listener
    // In a real implementation, you'd use Instagram's realtime API
    setInterval(async () => {
      try {
  ...
  async checkForNewMessages() {
    try {
      ...
      for (const thread of inbox) {
        ...
        for (const message of messages.slice(0, 1)) { // Only check latest message
